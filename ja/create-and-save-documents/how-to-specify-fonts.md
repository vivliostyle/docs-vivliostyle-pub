...
ただし、プレビューで使われるフォントと出力されたPDFに使われるフォントが、無条件に一致するのは上記のうち3だけです。これ以外の1、2では、お互いによく似たフォントが使われ、一致しない方が多いのが実際です。これはVivliostyle PubではプレビューとPDF出力は別々の機構が処理するからです。本節では、まずその仕組みについて概説します。ついでActionメニューの項目に沿いながら、3種類のフォントを利用する方法について説明していきます。
