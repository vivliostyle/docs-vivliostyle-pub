...
個々の`font-family`名は下記の通りです。これらをCustom theme（スタイルシート）の中で指定することにより、出力するPDFに埋め込むことができます（→[フォントを指定するしくみ](/ja/create-and-save-documents/how-to-specify-fonts.md#フォントを指定するしくみ)）
